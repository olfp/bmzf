...
zf_addr *uservar = (zf_addr *)dict;


/* Prototypes */

void do_prim(zf_prim prim, const char *input);
zf_addr dict_get_cell(zf_addr addr, zf_cell *v);
...
const char *op_name(zf_addr addr)
{
	zf_addr w = LATEST;
	static char name[32];
...
const char *op_name(zf_addr addr) { return NULL; }
#endif
...
zf_addr dict_put_cell_typed(zf_addr addr, zf_cell v, zf_mem_size size)
{
	unsigned int vi = v;
	uint8_t t[2];
...
zf_addr dict_get_cell_typed(zf_addr addr, zf_cell *v, zf_mem_size size)
{
	uint8_t t[2];
...
zf_addr dict_put_cell(zf_addr addr, zf_cell v)
{
...
void dict_add_cell_typed(zf_cell v, zf_mem_size size)
{
...
void dict_add_cell(zf_cell v)
{
	dict_add_cell_typed(v, ZF_MEM_SIZE_VAR);
}


void dict_add_op(zf_addr op)
{
	dict_add_cell(op);
	trace("+%s ", op_name(op));
}
...
zf_addr dict_add_str(const char *s)
{
	size_t l;
	zf_addr h;
...
int find_word(const char *name, zf_addr *word, zf_addr *code)
{
	zf_addr w = LATEST;
...
zf_addr peek(zf_addr addr, zf_cell *val, int len)
{
...
void do_prim(zf_prim op, const char *input)
{
	zf_cell d1, d2, d3;
	zf_addr addr, len;
...
void handle_word(const char *buf)
{
	zf_addr w, c = 0;
	int found;
