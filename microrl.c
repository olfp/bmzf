...
#endif
        strcpy (str, "\033[D");
	for(int i=0; i<_COMMAND_LINE_LEN + _PROMPT_LEN + 2; i++)
                pThis->print (str);
        strcpy (str, "\033[C");
	for(int i=0; i<_PROMPT_LEN; i++)
                pThis->print (str);
...
#endif
void microrl_set_execute_callback (microrl_t * pThis, int (*execute)(const char*))
{
